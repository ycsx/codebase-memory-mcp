...
export function fitCameraTargetToAspect(
  target: CameraTarget,
  aspect: number,
): CameraTarget {
  if (!Number.isFinite(aspect) || aspect >= 1) return target;

  const portraitScale = Math.min(2.25, 1 / Math.max(aspect, 0.45));
  const offset = target.position.clone().sub(target.lookAt).multiplyScalar(portraitScale);
  return {
    lookAt: target.lookAt.clone(),
    position: target.lookAt.clone().add(offset),
  };
}
...
function CameraAnimator({
  ...
  useEffect(() => {
    if (target) {
      targetRef.current = fitCameraTargetToAspect(target, size.width / size.height);
      progress.current = 0;
    }
