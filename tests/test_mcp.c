...
TEST(mcp_initialize_response) {
    ...
    ASSERT_NOT_NULL(strstr(json, "single-file mechanical edit"));
    ASSERT_NOT_NULL(strstr(json, "change impact"));
    ASSERT_NOT_NULL(strstr(json, "grep counts alone"));
...
TEST(mcp_usage_stats_aggregate_calls_by_file) {
    ...
    char *json = cbm_usage_stats_json(10);
    ASSERT_NOT_NULL(json);
    ASSERT_NOT_NULL(strstr(json, "\"available\":true"));
    ASSERT_NOT_NULL(strstr(json, "\"total_calls\":3"));
    ASSERT_NOT_NULL(strstr(json, "\"errors\":1"));
    ASSERT_NOT_NULL(strstr(json, "\"active_files\":2"));
    ASSERT_NOT_NULL(strstr(json, "\"unattributed_calls\":1"));
    ASSERT_NOT_NULL(strstr(json, "src/app.c"));
    ASSERT_NOT_NULL(strstr(json, "\"calls\":2"));
    free(json);

    char path[CBM_SZ_1K];
    snprintf(path, sizeof(path), "%s/_usage.db", cache);
    cbm_unlink(path);
    snprintf(path, sizeof(path), "%s/_usage.db-wal", cache);
    cbm_unlink(path);
    snprintf(path, sizeof(path), "%s/_usage.db-shm", cache);
    cbm_unlink(path);
    cbm_rmdir(cache);
...
TEST(tool_explain_impact_exact_symbol) {
    cbm_mcp_server_t *srv = cbm_mcp_server_new(NULL);
    cbm_store_t *st = cbm_mcp_server_store(srv);
    const char *proj = "impact-proj";
    cbm_mcp_server_set_project(srv, proj);
    cbm_store_upsert_project(st, proj, "/tmp/impact");

    cbm_node_t target = {.project = proj,
                         .label = "Function",
                         .name = "saveOrder",
                         .qualified_name = "impact-proj.orders.saveOrder",
    ...
    cbm_node_t caller = {.project = proj,
                         .label = "Function",
                         .name = "checkout",
                         .qualified_name = "impact-proj.checkout.checkout",
    ...
    cbm_node_t route = {.project = proj,
                        .label = "Route",
                        .name = "postCheckout",
                        .qualified_name = "impact-proj.routes.postCheckout",
                        .file_path = "src/routes.c",
                        .start_line = 4,
                        .end_line = 8};
    cbm_node_t test = {.project = proj,
                       .label = "Function",
                       .name = "testSaveOrder",
                       .qualified_name = "impact-proj.tests.testSaveOrder",
                       .file_path = "tests/orders_test.c",
                       .start_line = 3,
                       .end_line = 12};
    ...
    int64_t test_id = cbm_store_upsert_node(st, &test);
    ...
    cbm_edge_t e2 = {
        .project = proj, .source_id = route_id, .target_id = caller_id, .type = "CALLS"};
    cbm_edge_t e3 = {
        .project = proj, .source_id = test_id, .target_id = target_id, .type = "CALLS"};
    ...
    char *inner = extract_text_content(resp);
    ASSERT_NOT_NULL(inner);
    ASSERT_NOT_NULL(strstr(inner, "\"status\":\"analysis\""));
    ASSERT_NOT_NULL(strstr(inner, "\"direct_count\":2"));
    ASSERT_NOT_NULL(strstr(inner, "\"indirect_count\":1"));
    ASSERT_NOT_NULL(strstr(inner, "\"entry_points\":1"));
    ASSERT_NOT_NULL(strstr(inner, "\"tests\":1"));
    ASSERT_NOT_NULL(strstr(inner, "\"relationship\":\"CALLS\""));
    free(inner);
    free(resp);
    cbm_mcp_server_free(srv);
    PASS();
...
TEST(tool_explain_impact_ambiguous_candidates) {
    cbm_mcp_server_t *srv = cbm_mcp_server_new(NULL);
    cbm_store_t *st = cbm_mcp_server_store(srv);
    const char *proj = "impact-ambiguous";
    cbm_mcp_server_set_project(srv, proj);
    cbm_store_upsert_project(st, proj, "/tmp/impact-ambiguous");
    cbm_node_t first = {.project = proj,
                        .label = "Function",
                        .name = "renderPanel",
                        .qualified_name = "impact-ambiguous.admin.renderPanel",
    ...
    cbm_node_t second = {.project = proj,
                         .label = "Function",
                         .name = "renderPanel",
                         .qualified_name = "impact-ambiguous.user.renderPanel",
    ...
    char *inner = extract_text_content(resp);
    ASSERT_NOT_NULL(inner);
    ASSERT_NOT_NULL(strstr(inner, "\"status\":\"candidates\""));
    ASSERT_NOT_NULL(strstr(inner, "impact-ambiguous.admin.renderPanel"));
...
TEST(tool_explain_impact_file_aggregates_symbols) {
    cbm_mcp_server_t *srv = cbm_mcp_server_new(NULL);
    cbm_store_t *st = cbm_mcp_server_store(srv);
    const char *proj = "impact-file";
    cbm_mcp_server_set_project(srv, proj);
    cbm_store_upsert_project(st, proj, "/tmp/impact-file");
    cbm_node_t file_symbol = {.project = proj,
                              .label = "Function",
                              .name = "sendChat",
                              .qualified_name = "impact-file.utils.chat.sendChat",
                              .file_path = "utils/chat.js",
                              .start_line = 1,
                              .end_line = 15};
    cbm_node_t caller = {.project = proj,
                         .label = "Function",
                         .name = "submitMessage",
                         .qualified_name = "impact-file.components.submitMessage",
                         .file_path = "components/Composer.js",
                         .start_line = 1,
                         .end_line = 8};
    int64_t symbol_id = cbm_store_upsert_node(st, &file_symbol);
    int64_t caller_id = cbm_store_upsert_node(st, &caller);
    ASSERT_GT(symbol_id, 0);
    ASSERT_GT(caller_id, 0);
    cbm_edge_t edge = {
    ...
    ASSERT_NOT_NULL(inner);
    ASSERT_NOT_NULL(strstr(inner, "\"status\":\"analysis\""));
    ASSERT_NOT_NULL(strstr(inner, "\"target_type\":\"file\""));
    ASSERT_NOT_NULL(strstr(inner, "\"direct_count\":1"));
    ASSERT_NOT_NULL(strstr(inner, "impact-file.components.submitMessage"));
    free(inner);
    free(resp);
    cbm_mcp_server_free(srv);
    PASS();
...
TEST(tool_explain_impact_filters_docs_and_merges_module_file_candidates) {
    cbm_mcp_server_t *srv = cbm_mcp_server_new(NULL);
    cbm_store_t *st = cbm_mcp_server_store(srv);
    const char *proj = "impact-candidates";
    cbm_mcp_server_set_project(srv, proj);
    cbm_store_upsert_project(st, proj, "/tmp/impact-candidates");
    cbm_node_t docs = {.project = proj,
                       .label = "Section",
                       .name = "SSE parser (utils/chat.js)",
                       .qualified_name = "impact-candidates.docs.sse-parser",
                       .file_path = "skills/frontend/SKILL.md"};
    cbm_node_t module = {.project = proj,
                         .label = "Module",
                         .name = "src/utils/chat.js",
                         .qualified_name = "impact-candidates.src.utils.chat",
                         .file_path = "src/utils/chat.js"};
    cbm_node_t file = {.project = proj,
                       .label = "File",
                       .name = "chat.js",
                       .qualified_name = "impact-candidates.file.src.utils.chat",
                       .file_path = "src/utils/chat.js"};
    cbm_node_t other = {.project = proj,
                        .label = "Module",
                        .name = "packages/other/utils/chat.js",
                        .qualified_name = "impact-candidates.packages.other.utils.chat",
                        .file_path = "packages/other/utils/chat.js"};
    ASSERT_GT(cbm_store_upsert_node(st, &docs), 0);
    ASSERT_GT(cbm_store_upsert_node(st, &module), 0);
    ASSERT_GT(cbm_store_upsert_node(st, &file), 0);
    ...
    ASSERT_NOT_NULL(inner);
    ASSERT_NOT_NULL(strstr(inner, "\"status\":\"candidates\""));
    ASSERT_NOT_NULL(strstr(inner, "file:src/utils/chat.js"));
    ASSERT_NOT_NULL(strstr(inner, "file:packages/other/utils/chat.js"));
    ASSERT_NULL(strstr(inner, "impact-candidates.docs.sse-parser"));
    ASSERT_NULL(strstr(inner, "impact-candidates.src.utils.chat"));
    free(inner);
    free(resp);
    cbm_mcp_server_free(srv);
    PASS();
...
SUITE(mcp) {
    ...
    RUN_TEST(mcp_usage_stats_aggregate_calls_by_file);
