...
static const char skill_content[] =
    ...
    "| Task context package | `build_context(task=..., target=..., token_budget=...)` |\n"
    "| Document to code | `get_document(project=..., path=\"docs/design.md\")` |\n"
    "| Code to documents | `get_related_documents(project=..., target=\"file:src/core.c\")` |\n"
    ...
    "\n"
    "## Document Evidence\n"
    "- `get_document` accepts a repository-relative path or document name; returns sections, "
    "up to 100 references, `references_truncated`, and `reference_analysis`.\n"
    "- `get_related_documents` requires an exact qualified symbol name or "
    "`file:repository/relative/path`. File targets include symbols in that file. "
    "`limit` defaults to 20 (maximum 100); check total/returned/truncated and analysis status.\n"
    "- Set `include_docs=true` on `build_context` or `explain_impact` to include document "
    "evidence (default false). `review_change` defaults to true; set false to opt out. "
    "`explain_impact.document_limit` defaults to 20 (maximum 100). Context/review output "
    "is also constrained by `token_budget`; inspect truncation and limitations.\n"
    "- REFERENCES records deterministic explicit Markdown links, inline file paths, and exact "
    "qualified symbols, not semantic relevance or complete CommonMark parsing. Evidence includes "
    "document lines, matched text, and target; no short-name guessing. Empty results or "
    "`status=ok` do not prove complete documentation coverage. Legacy indexes may require "
    "reindexing; verify limited/unknown analysis against the source.\n"
    "\n"
    "- `get_document_coverage` lists indexed code/document coverage with bounded pagination. "
    "Unreferenced code does not imply missing documentation.\n"
    "- `update_document_review` confirms or reopens a reference using its current review token. "
    "This writes local review state; confirmation is not a claim that documentation is correct. "
    "Changed code or document content requires another review. Refresh after a stale-token error.\n"
    "\n"
    "## 22 MCP Tools\n"
    "`index_repository`, `index_status`, `list_projects`, `delete_project`,\n"
    "`search_graph`, `search_code`, `trace_path`, `detect_changes`,\n"
    "`query_graph`, `get_graph_schema`, `get_code_snippet`, `get_architecture`, `explain_impact`,\n"
    "`build_context`, `review_change`, `check_index_coverage`, `manage_adr`, `ingest_traces`,\n"
    ...
    "CONTAINS_PACKAGE, CONTAINS_SECTION, REFERENCES\n"
    "\n"
...
static const char codex_instructions_content[] =
    ...
    "- `get_document` — indexed Markdown sections and explicit code-reference evidence\n"
    "- `get_related_documents` — documents referencing an exact qualified symbol or "
    "`file:repository/relative/path` (including its symbols)\n"
    "- `get_document_coverage` — paginated indexed code/document coverage, not a documentation "
    "completeness guarantee\n"
    "- `update_document_review` — explicit local review confirmation or reopening with a current "
    "token; a write operation, not available in read-only profiles\n"
    "\n"
    "`include_docs` defaults to false in `build_context`/`explain_impact`, true in "
    "`review_change`. Check document analysis status, evidence, and truncation; empty results "
    "are not proof of complete documentation coverage. Legacy indexes may need reindexing.\n"
    "\n"
...
static const char agent_instructions_content[] =
    ...
    "\n"
    "### Document references\n"
    "- `get_document(project=..., path=\"docs/design.md\")` returns indexed sections and "
    "up to 100 explicit references; check `references_truncated` and `reference_analysis`.\n"
    "- `get_related_documents(project=..., target=\"file:src/core.c\")` finds referencing "
    "documents/sections. Use an exact qualified symbol or `file:repository/relative/path`; "
    "file targets include their symbols. `limit` defaults to 20, maximum 100.\n"
    "- `build_context` and `explain_impact` default `include_docs=false`; enable it explicitly. "
    "`review_change` defaults to true. Respect budgets, truncation, and analysis limitations.\n"
    "- Evidence is deterministic explicit Markdown references, not semantic relevance or "
    "complete Markdown coverage. Empty results and `status=ok` do not prove completeness; "
    "inspect source for limited/unknown results and reindex legacy documents when required.\n"
    "\n"
    "### Evidence tiers\n"
...
static const char aider_instructions_content[] =
    ...
    "\n"
    "## Document references (CLI form)\n"
    "Read indexed document sections and code references:\n"
    "   codebase-memory-mcp cli get_document "
    "'{\"project\":\"<name>\",\"path\":\"docs/design.md\"}'\n"
    "Find documents referencing a file (including its symbols) or an exact qualified symbol:\n"
    "   codebase-memory-mcp cli get_related_documents "
    "'{\"project\":\"<name>\",\"target\":\"file:src/core.c\",\"limit\":20}'\n"
    "`get_document` caps references at 100; reverse lookup defaults to 20 (maximum 100). "
    "Check reference analysis and truncation. Explicit Markdown evidence is best-effort, not "
    "semantic relevance; empty results do not prove complete documentation coverage.\n"
    "Set `include_docs` to true for `build_context`/`explain_impact` (default false); "
    "`review_change` defaults to true. Respect token budgets and analysis limitations.\n"
    "\n"
