...
The UI is embedded in the `--with-ui` binary. Start it with:

```powershell
$env:HOME = 'C:\Users\Public\cbm-home'
$env:USERPROFILE = 'C:\Users\Public\cbm-home'
& '.\build\c\codebase-memory-mcp.exe' --ui=true --port=9750
```

Keep the terminal open and visit <http://127.0.0.1:9750/>. Use a different
`HOME` and `USERPROFILE` pair when an isolated index database is required.
