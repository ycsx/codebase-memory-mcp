...
static int publish_writer_output(write_db_ctx_t *w) {
    ...
    int replace_rc = cbm_rename_replace(w->temp_path, w->final_path);
#ifdef _WIN32
    /* SQLite readers on Windows do not share the database handle for delete,
     * so even a millisecond-long UI stats query can make MoveFileEx fail. The
     * complete database is already durable in the adjacent temp file here;
     * retry briefly to let transient readers close without weakening the
     * atomic replacement contract. A genuinely persistent reader still times
     * out and follows the existing preserve-old-database failure path. */
    enum { PUBLISH_RETRY_COUNT = 40, PUBLISH_RETRY_DELAY_MS = 50 };
    for (int attempt = 0; replace_rc != 0 && attempt < PUBLISH_RETRY_COUNT; attempt++) {
        Sleep(PUBLISH_RETRY_DELAY_MS);
        replace_rc = cbm_rename_replace(w->temp_path, w->final_path);
    }
#endif
    if (replace_rc != 0) {
        (void)cbm_unlink(w->temp_path);
