...
[![VirusTotal](https://img.shields.io/badge/VirusTotal-best_effort-informational?logo=virustotal)](SECURITY.md)
...
|----------|---------|-------------|
| `CBM_ALLOWED_ROOT` | *(unset)* | Confine MCP and graph UI indexing to paths within this directory. Unset imposes no containment restriction, but filesystem roots, top-level system trees, home directories, and credential directories are always refused as indexing roots. |
...
Published release binaries are protected by this multi-layer pipeline before publication:

- **VirusTotal** — release binaries are submitted on a best-effort basis; transient service failures warn without blocking by default, while release operators can select required or skip mode
