...
> 状态（2026-09-20）：文档双向引用、组合工具、基础覆盖矩阵与持久内容版本复核已交付并通过本地验证；人工质量、完整漂移及历史引用仍待推进
...
2026-09-20 已补齐反向查询与三项组合工具接入，并以 `14692b6e` 为本轮基线，
依次交付完整文档试用、文件级复核提醒和代码详情/评审最小文档 UI。
当前实现与限制见 9.5、第 10-12 节；其余目标架构、数据模型和阶段交付物仍是规划。
63 条候选在单行与完整上下文中均通过，但标注仍待维护者复核，
不代表整个 W11/W12 或完整文档知识图谱已经验收。
...
- 已能对提及变更文件的文档给出“建议复核”，但不判断文档过期，也不保留历史漂移状态。
- 无法衡量高风险代码是否有足够文档覆盖。
...
同日新增 `scripts/eval-document-links-full.py`，复制 923 个 Git tracked 真实文件，
使用隔离仓库和缓存进行 stdio 索引，8 份完整原文的 63/63 候选通过。
评测不使用目标 stub，明确排除 vendor 目录及大于 2 MB 的文件。
七处重复引用采用更早证据行，一份文档保留 `limited/unsupported_multiline_code`；
原摘录期望未更改。两套评测器 19 项 Python 单测通过。
[完整报告](reports/DOCUMENT_REFERENCES_FULL_2026-09-20.md)记录解析缺口和排除范围，
仍不构成人工确认黄金集或全仓精确率证明。

本轮提醒及 UI 聚焦测试为 206 项 C 通过、6 项平台跳过，真实 stdio 提醒 fixture 通过，
93 项前端测试及构建通过；最小 UI 的 1440x1000 桌面与 390x844 移动浏览器 QA 通过，
覆盖评审提醒、函数/文件反查及 Markdown 第 5 行高亮阅读，详细环境、证据及已有警告见验收台账。
这些计数与前述历史轮次分开记录，不相加作为全量测试结论。

## 10. 文档漂移分析

当前已实现下述文件级提醒和内容版本复核；10.1-10.3 的完整自动漂移检测及全部状态仍是规划，
不应将引用命中或空结果转换为过期/无覆盖结论。

### 当前最小复核提醒

`review_change` 成功读取变更且启用文档输出时，为命中的代码引用附带 `review`：
`status:"review"`、`reason:"referenced_file_changed"`、触发文件、目标 QN、
来源文档是否同时修改，以及不等同于“文档已过期”的提示。
这是文件级信号，不能证明某个具体函数改变；文档也被修改不等于已经正确同步。

`related_documentation.review_basis` 保留请求的比较基准、Git 比较口径、
`current_index` 引用快照来源与 `current/stale/unknown` 新鲜度。
纯文档变更不触发代码复核提醒；提醒不提升风险等级或新增阻断规则，
字段开销计入原有证据预算。当前索引已清理的删除/重命名历史引用无法恢复。
当前没有历史图谱比较、完整自动漂移、用户忽略策略或自动文档修改。

### 内容版本复核（已实现，本地验证通过）

`update_document_review` 使用完整来源文件与目标文件的当前字节指纹绑定复核内容版本，
支持用户确认及重新打开。文件内容变动后，不把旧版本确认当作新版本已确认；
内容恢复到此前已确认版本时，可以恢复对应确认。
UI 使用服务端提供的 token 提交操作，不自行构造版本身份。

记录写入独立 `.reviews.sqlite` SQLite sidecar，与图谱重建分离，
因此普通图谱重建不清除已保存的复核记录。该记录不保存已删除的历史引用边，
不证明文档足够完整、正确或永不过期，也不是本节规划的自动 `stale/orphan` 判定。
新增浏览器、独立生命周期与 C 回归通过；本轮结果与上一轮最小 UI 分开记录。
C 最终 724 passed、13 项平台 skipped、退出码 0，Windows/MSYS Clang 构建通过，
显式 `SANITIZE=`，未运行 ASan/UBSan。测试覆盖重索引期间交错确认、取消重试、
缓存来源变更和 sidecar 删除隔离，日志为 `.tmp/document-workflow-tests-native.log`
及 `.tmp/document-workflow-build.log`。
初次测试缺少 `platform.h` 已作测试侧修正；MSYS TEMP 首轮 fixture 失败后，
同一已编译二进制在原生 PowerShell 的独立 ASCII 临时目录下重跑通过，
没有生产绕过。完整失败计数和 Unicode 缓存独立验证边界见验收台账。
本仓及本地 `aikb-web` 的真实有界副本共 534 文件、54 文档（269/29 与 265/25），
原项目哈希不变；后者 `skills/vue-iview-spec/SKILL.md` 保留
`limited/unsupported_multiline_code`。独立生成生命周期 fixture 11 场景通过，
涵盖重启、重索引、代码/文档/共同编辑、旧版本、删除、重命名和隔离。
Python 30 项中 29 通过、1 Windows symlink 跳过，完整文档回归重跑 63/63。
证据 `.tmp/document-workflow-verified.json` 使用 Unicode 临时根目录
`C:/Temp/cbm-doc-复核`，这些真实项目试用和生成 fixture 不代替人工黄金标注。
...
| `review_change` | `include_docs` 默认 true；按变更路径返回 `related_documentation`、文件级 `review` 与 `review_basis`，并计入估算预算 |
| `get_document_coverage` | 已实现并通过本地验证：查询已索引非 Markdown File 与文档引用状态，提供分页；不衡量文档充分性或全仓完整覆盖 |
| `update_document_review` | 已实现并通过本地验证：按来源/目标完整文件当前内容版本确认或重新打开，持久化在独立 sidecar |
...
- 普通相关文档不等于漂移提示；`review_change` 的文件级“建议复核”也不等于过期结论。需回读原文和代码后决定是否更新，空结果不得解释为无文档。

## 12. UI 信息架构规划

本轮已交付最小子集：代码节点详情显示相关文档与引用证据，评审结果显示相关文档和
文件级复核提醒，均可打开原文并高亮命中行，包含加载、错误、空状态及请求竞争处理。
前端 93 项测试和构建通过；普通 Playwright fallback 已在真实本地 console/Vite、
隔离 Git fixture 上完成 1440x1000 与 390x844 验证：评审提醒到函数定位、
无直接引用状态到文件详情反查，再到原文第 5 行高亮。
移动无页面横向溢出，未见应用 console error 或框架错误遮罩；
已有 favicon 404 和两次 `THREE.Clock` deprecated 为单列警告。
图谱 canvas 为 854x913，包含 3,741 种颜色、143,754 个非背景像素；
非空及滚轮缩放截图变化检查通过。证据位于
`C:/Temp/cbm-w12-qa/browser-result.json` 及同目录桌面/移动截图，范围不等于完整 W12 UI 验收。
同日后续新增基础覆盖矩阵、代码/文档筛选、分页、原文阅读和服务端 token
确认/重新打开操作，本地验证通过；前端空闲重跑 103/103 与构建通过，
并发首轮既有 `StatsTab` 断言波动见台账。新增 C 回归 724 通过、13 项平台跳过。
新增浏览器工作流已通过普通 Playwright（无 Browser 插件），真实 console `9749` /
Vite `5173` 上的 1440x1000 与 390x844 覆盖：
筛选、打开引用、确认、重载保持确认、重新打开、编辑后旧 token 拒绝、刷新后确认、
再次编辑回到待复核、原文阅读及移动无横向溢出。
无应用错误或框架遮罩，已有 favicon 404 和两次 `THREE.Clock` 弃用警告单列。
证据为 `C:/Temp/cbm-w12-qa/workflow-browser-result.json` 及同目录
`workflow-desktop.png`、`workflow-mobile.png`；不代表完整 W12 或远程验收。
以下分节区分基础矩阵与仍未交付的混合图层、
风险覆盖评分和知识健康页面，上一轮 QA 不自动覆盖新增 UI。
...
待实现：增加 `代码 | 文档 | 混合` 分段控制：

- `代码`：保持当前默认体验。
- `文档`：显示文档层级和文档间关系。
- `混合`：只显示当前选中对象附近的跨层关系，不一次加载全部文档节点。

已实现最小代码节点详情区域：相关文档/章节、目标和来源证据、命中行原文阅读。
下列扩展字段与完整关系路径交互仍是规划，不应把引用状态当作漂移状态：
...
已实现评审结果中的相关文档、文件级“建议复核”、触发证据和原文阅读。
下列完整影响分类与独立影响页增强仍为规划；当前不会输出“必须更新”或断言引用已经失效：
...
已实现并通过本地验证：基础文档覆盖视图按当前已索引非 Markdown File 与文档状态展示，
支持代码/文档筛选、分页和原文阅读。它不把未命中引用解释为无文档，
不把当前索引集合解释为整个仓库，也不将存在引用解释为说明充分。

下列结合热点和质量评分的完整“代码风险与文档覆盖矩阵”仍是规划：
...
当前状态（2026-09-20）：文件/完整 QN 引用、证据、双向查询和节点详情最小相关知识 UI 已实现；路由/配置匹配、候选列表、统一搜索仍未实现；人工质量门槛仍待验收。
...
当前状态（2026-09-20）：三项组合工具、文件级提醒和最小 UI 已交付；
基础覆盖矩阵与持久内容版本复核已实现并通过本地验证；
下列 `detect_changes` 字段、完整影响页、风险覆盖矩阵和混合图层仍是后续交付物。
...
当前子集：基于来源/目标完整文件当前字节指纹的确认/重新打开及独立 sidecar 持久化
已实现并通过本地验证；这不代表以下全部漂移状态、语义候选、历史引用保留或忽略策略已经实现。
...
2. 保留已通过的本仓完整原文 63 条及本仓/本地 `aikb-web` 有界副本试用；生成生命周期 fixture 单列，不作为真实人工样本。继续扩展第三个项目及代表性精确率/召回率。
3. 保留新增覆盖查询与内容版本复核的 C 724/13 平台跳过、前端 103/103、构建、浏览器及 11 场景生命周期回归，继续做跨平台验收。
4. 保留最小 UI 回归，在基础覆盖矩阵上评估风险评分、独立影响页和知识健康摘要。
5. 评估历史删除引用保留及完整自动漂移；已有内容版本确认不等于文档过期或质量结论。
