...
同一项目可以分别建立本地与 Git 图谱。新建 Git 索引时，项目 ID 留空会按仓库名生成；如果同名图谱或目录已被占用，会依次尝试 `仓库名-git`、`仓库名-git-2` 等名称。现存托管克隆只有 URL 和分支都匹配时才会复用，本地代码目录不会被接管。手动填写的项目 ID 不会自动改名，遇到冲突时请换一个 ID，或清空该字段让系统自动区分。
