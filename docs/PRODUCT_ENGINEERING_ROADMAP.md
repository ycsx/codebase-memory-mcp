...
> 状态（2026-09-20）：本地文档双向引用、组合工具、文件级复核提醒及最小文档 UI 已实现；整阶段验收见台账
...
> 当前验收状态：[里程碑验收台账](MILESTONE_ACCEPTANCE.md)；周次统一采用原 20 周排期
...
当前 MCP 工具包括原子查询和组合分析能力，实际清单以对应 profile 的 `tools/list` 为准，主要覆盖：
...
已交付的 MVP：`build_context` 提供有预算的任务证据包，`review_change` 将 Git diff、静态影响、测试/文档缺口和规则状态汇总为确定性评审结果，并接入图谱 UI 的“评审”模式；`scripts/review-change-comment.py` 负责按 commit 去重的 GitHub/GitLab 评论、CODEOWNERS Owner 展示和可选门禁。远程 MCP 已补齐托管 Key、主体、项目 ACL、工具档位、独立写权限、会话绑定和审计字段；本机控制面已提供项目目录、索引任务、健康探针和指标；Markdown 已进入文档/章节图谱并提供 `get_document` 查询。真实 PR 影子评审和规则确认/忽略遥测仍需目标项目采样；远程任务持久化仍待实现与验收。

2026-09-20 本地进展：明确文件路径和完整 qualified name 已生成带行号证据的 `REFERENCES`；`get_related_documents` 已支持文件/完整符号反查文档；`build_context`、`explain_impact`、`review_change` 已按需接入相关文档及数量、预算或截断信息。引用表示原文提及代码，不等于过期判断或覆盖证明。远程 HTTP 验收暂缓，不阻塞本地功能开发，也不据此宣布远程阶段通过。

本轮验证记录：C 测试 255 通过、6 项平台跳过，上下文回归 31/31，文档摘录回归 63/63，评测脚本 Python 测试 11 项通过。63 条来自 8 个真实 Markdown 文件的单行摘录（33 正例、30 反例），目标使用 File stub，标注仍待维护者复核；不是全仓库精确率/召回率，也不代表 W11、W12 或文档 UI 全部完成。

同日以 `14692b6e` 为基线，按原排期 W12 的本地范围追加部分闭环：
完整文档试用复制 923 个 Git tracked 真实文件，8 份原文的 63/63 候选通过，
明确排除 vendor 目录及大于 2 MB 文件，两套评测器共 19 项 Python 测试通过。
`review_change` 增加文件级“建议复核”，保留触发证据、比较口径、当前索引来源和新鲜度；
不判断文档过期、不改变风险等级，也不能恢复当前索引已清理的历史引用。
代码详情和评审结果已展示相关文档、原文阅读与证据行高亮，含请求竞争和错误处理。
本轮聚焦 C 测试 206 通过、6 项平台跳过，真实 stdio 提醒 fixture 通过，
前端 93 项测试与构建通过；普通 Playwright fallback 的最小 UI 浏览器 QA 通过，
在真实本地 console/Vite 与隔离 Git fixture 上覆盖 1440x1000、390x844：
评审提醒、函数/文件反查、原文第 5 行高亮；移动无页面横向溢出，
...
及同目录桌面/移动截图；不是完整 W12 UI 验收。
[完整文档报告](reports/DOCUMENT_REFERENCES_FULL_2026-09-20.md)保留范围限制；
以上是上一轮结果；同日后续新增基础覆盖矩阵与持久内容版本复核，本地验证通过。
`get_document_coverage` 对已索引非 Markdown File 与文档状态提供分页查询；
`update_document_review` 以完整来源/目标文件当前字节指纹绑定确认或重新打开操作，
状态存入独立 `.reviews.sqlite` sidecar，图谱重建不清除确认，
恢复为已确认内容版本时可恢复确认。UI 已实现代码/文档筛选、分页、原文阅读和服务端 token 操作。
前端空闲重跑 103/103 通过、构建通过；并发首轮既有 `StatsTab` 断言波动见台账。
本仓/本地 `aikb-web` 有界副本试用通过：269/265 文件、29/25 文档，
原项目哈希不变，1 份文档保留多行代码限制；独立生成生命周期 fixture 11 场景通过。
Python 30 项中 29 通过、1 Windows symlink 跳过，完整文档回归重跑 63/63。
真实本地 console/Vite 的 1440x1000/390x844 浏览器流程通过：
覆盖筛选、确认跨重载保持、重新打开、旧 token 拒绝、新内容重新确认及再次编辑待复核、
原文阅读和移动无溢出；无应用错误/框架遮罩，已有 favicon 404 和两次 `THREE.Clock` 警告保留。
证据见台账、`.tmp/document-workflow-verified.json`
与 `C:/Temp/cbm-w12-qa/workflow-browser-result.json`。
新增 C 回归最终 724 passed、13 项平台 skipped、退出码 0，Windows/MSYS Clang
构建通过（`SANITIZE=`，未运行 ASan/UBSan）；重索引交错确认、取消重试和 sidecar
隔离均纳入测试。初次测试头文件修正及 MSYS TEMP 导致 fixture 失败、
同一二进制在原生 PowerShell 重跑通过的过程见台账，无生产绕过。
最终日志为 `.tmp/document-workflow-tests-native.log`、`.tmp/document-workflow-build.log`。
完整自动漂移、历史删除边恢复、风险覆盖评分及 >=95% 人工黄金集门槛仍未完成。
...
- 远程任务历史目前保存在进程内固定容量数组，重启后不会保留历史。
- Git 托管克隆、同步和本地/Git 项目 ID 冲突保护已有实现，仍需目标部署上的恢复与隔离验收。
- 配额/限流、查询缓存和优先级队列需要按团队试点门槛逐项验收，不能用现有会话上限代替请求限流。
- 完整远程管理 UI、OIDC/团队身份和审计检索接口仍属于后续阶段。
- 索引接口已有队列与去重；任务持久化、重启恢复和并发压力下的行为仍需单独验收。
- 没有服务健康、请求延迟、会话、索引队列和错误率管理 UI。
- 高层响应已有统一可信元数据契约，仍需持续回归过期、缺口和截断状态。
- Markdown 结构、双向引用、组合工具、文件级提醒、基础覆盖矩阵与持久内容版本复核已交付并通过本地验证。路由/配置关联、人工质量基线、历史删除边、完整漂移和风险覆盖评分仍待推进。需求、事故、Owner 和 CI 结果的完整知识建模仍待推进。
...
- 已实现：路径和完整 qualified name 引用、`get_related_documents` 反向查询。
- 已实现：`build_context`、`explain_impact`、`review_change` 相关文档证据接入。
- 已实现：文件级“建议复核”、代码节点详情及评审结果的相关文档与原文证据阅读。
- 已验证子集：本仓 8 份完整原文的 63 条候选；有明确 vendor/大文件排除和部分分析限制。
- 已实现并通过本地验证：已索引文件/文档基础覆盖矩阵与分页、基于完整文件字节指纹的持久确认/重新打开复核。
- 待实现：路由/配置键关联、完整影响页分类、风险覆盖评分、自动漂移及历史删除引用恢复。
- 待验收：维护者复核标注、跨项目代表性质量、原 W11 精确率门槛和完整 W12 UI；本轮最小 UI 桌面/移动流程已通过。
...
文档关系已增强 `build_context`、`explain_impact` 和 `review_change`；迁移规划和故障解释接入仍属后续计划，不应将已有引用结果视为漂移结论。

## 9. 图片候选能力评估

### 9.1 总览

下表保留候选能力的规划比较；已交付进度以上文当前基线和验收台账为准。

| 能力 | 当前基础 | 主要缺口 | 产品价值 | 工程量 | 决策 |
|---|---|---|---:|---:|---|
| 可信响应契约 | 覆盖缺口、索引状态、证据字段已有部分基础 | generation、新鲜度和统一策略 | 5/5 | M | P0 立即做 |
| `build_context` | 已有原子查询、搜索、影响、ADR | 真实任务校准、排序与预算持续验证 | 5/5 | M | P1 主功能 |
| `review_change` | diff、影响、复杂度、测试节点 | 规则、前后对比、Owner、CI | 5/5 | M-L | P1 主功能 |
| 文档知识图谱 | 文档结构、双向引用、组合工具、提醒与 UI、基础覆盖和持久内容版本复核 | 人工质量、关系扩展、自动漂移、历史边与风险覆盖 | 5/5 | L | P1-P2 主线 |
...
以下保留原路线图行动项，不表示每项仍未实现，原 W1-W20 顺序不变。
2026-09-20 已完成本仓完整原文试用、文件级复核提醒和代码详情/评审最小文档 UI；
下一步是保留已通过的 C、浏览器、本仓/本地 `aikb-web` 有界副本和生成生命周期
fixture 独立证据，由维护者复核
63 条标注并建立代表性跨项目质量基线。历史删除边、完整自动漂移与风险覆盖评分
仍需独立设计和验收。已实现能力不再列为待开发，远程 HTTP 验收保持暂缓。
