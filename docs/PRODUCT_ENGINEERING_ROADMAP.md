...
> 状态：W3/W4 本地能力与 CI 评论适配器已交付，真实 PR 影子评审和规则遥测进入运营验收
...
当前 `TOOLS[]` 已经包含 18 个原子 MCP 工具，主要覆盖：
...
W3/W4 已交付：`build_context` 提供有预算的任务证据包，`review_change` 将 Git diff、静态影响、测试/文档缺口和规则状态汇总为确定性评审结果，并接入图谱 UI 的“评审”模式；`scripts/review-change-comment.py` 负责按 commit 去重的 GitHub/GitLab 评论、CODEOWNERS Owner 展示和可选门禁。真实 PR 影子评审、规则确认/忽略遥测需要在目标仓库接入后采样。
