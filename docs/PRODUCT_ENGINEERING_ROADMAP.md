...
> 状态（2026-09-20）：本地文档双向引用与上下文/影响/评审接入已实现；整阶段验收见台账
...
- Markdown 文档/章节、明确路径和完整 QN 的双向引用及组合工具接入已实现；路由/配置关联、人工复核质量基线、真实整篇文档验证和漂移/覆盖 UI 仍待推进。需求、事故、Owner 和 CI 结果的完整知识建模仍待推进。
...
- 待实现：路由/配置键关联、代码节点“相关知识”、影响页“受影响文档”和热点页覆盖矩阵。
- 待验收：维护者复核标注、整篇真实文档质量与原 W11 精确率门槛。
...
| 文档知识图谱 | Document/Section、双向引用、组合工具接入 | 人工质量复核、关系扩展、漂移、覆盖 UI | 5/5 | L | P1-P2 主线 |
...
以下保留原路线图行动项，不表示每项仍未实现。2026-09-20 当前本地主线是复核 63 条摘录标注、扩充整篇文档与真实变更评测，再推进相关知识/影响/覆盖 UI；Markdown 生命周期、首批引用与三项组合工具接入不再列为待开发。远程 HTTP 验收保持暂缓。
