...
当前源码内置大规模 Tree-sitter grammar 注册表，覆盖主流编程语言以及配置、模板、基础设施和领域语言。语言枚举数量会随分支扩展；对外展示时以当前二进制 `get_architecture` 返回的语言统计为准，不把历史评估计划中的语言数当作运行时保证。
...
- 统一语言统计、MCP 工具数量、测试数量、算法名称等文档口径；README 只保留已由当前实现或发布流程确认的数字。
